...
class Database:
    ...
    async def list_feedback(self, *, status: str | None = None,
    ...
    async def delete_feedback(self, feedback_id: int) -> bool:
        cursor = await self._db.execute("DELETE FROM feedback WHERE id = ?", [feedback_id])
        await self._db.commit()
        return (cursor.rowcount or 0) > 0

    # --- Title suggestions ---

    async def add_title_suggestion(self, *, username: str, query: str,
                                   ...
                                   catalog_token: str | None = None) -> int:
        cursor = await self._db.execute(
    ...
    async def list_title_suggestions(self, *, status: str | None = None,
                                     limit: int = 100, offset: int = 0) -> list[dict]:
    ...
    async def set_title_suggestion_status(self, suggestion_id: int, status: str) -> bool:
        cursor = await self._db.execute(
            "UPDATE title_suggestions SET status = ? WHERE id = ?", [status, suggestion_id]
        )
        await self._db.commit()
        return (cursor.rowcount or 0) > 0
