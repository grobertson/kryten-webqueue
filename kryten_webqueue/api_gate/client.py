...
class ApiGateClient:
    ...
    async def send_pm(self, username: str, message: str) -> dict:
        return await self.post(
            "/chat/pm", json={"username": username, "message": message}
        )
    ...
    async def get_transactions(
        ...
        return await self.get(
            f"/economy/transactions/{username}", limit=limit, offset=offset
        )
    ...
    async def queue_preview(
        self, username: str, duration_sec: int, tier: str = "queue"
    ) -> dict:
        return await self.post(
            "/economy/queue-preview",
            json={
                "username": username,
    ...
    async def queue_spend(
        self, username: str, duration_sec: int, tier: str, request_id: str
    ) -> dict:
        return await self.post(
            "/economy/queue-spend",
            json={
    ...
    async def mod_list_entries(
        self, channel: str, action_filter: str | None = None
    ) -> dict:
        params: dict = {}
        if action_filter:
    ...
    async def mod_recent_users(
        self, channel: str, window_minutes: float = 60.0
    ) -> dict:
        return await self.get(
