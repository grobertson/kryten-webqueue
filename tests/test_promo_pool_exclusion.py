...
async def test_promo_pool_clips_excluded_from_browse_search_and_pay(db):
    ...
    pool_id = await db.create_saved_playlist(
        name="Channel IDs", description=None, is_immutable=False,
        created_by="admin", promo_type="channel_identity",
    )
    await db.append_playlist_item(
        pool_id, {"media_type": "cm", "media_id": "promotok", "title": "Sparkle Promo Clip", "duration_sec": 600},
    )
...
async def test_clearing_promo_type_restores_visibility(db):
    ...
    pool_id = await db.create_saved_playlist(
        name="Events", description=None, is_immutable=False,
        created_by="admin", promo_type="event",
    )
    await db.append_playlist_item(
        pool_id, {"media_type": "cm", "media_id": "clip1", "title": "Reusable Clip", "duration_sec": 600},
    )
    ...
    await db.update_saved_playlist(
        pool_id, name="Events", description=None, is_immutable=False, promo_type=None,
    )
