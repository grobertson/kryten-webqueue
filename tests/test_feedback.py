...
from kryten_webqueue.routes.feedback import (
    FeedbackSubmit, SuggestResolve, SuggestSubmit, SuggestChoice,
    submit_feedback, resolve_suggestion, submit_suggestion,
)
...
async def test_title_suggestion_crud(db):
    sid = await db.add_title_suggestion(
        username="Bob", query="the matrix", resolved_title="The Matrix",
        resolved_year="1999", resolved_source="tmdb", resolved_id="603",
        poster_url="http://x/p.jpg", resolution="resolved",
    )
    ...
    if "search/movie" in url:
        return _FakeResp({"results": [
            {"id": 603, "title": "The Matrix", "release_date": "1999-03-30",
             "poster_path": "/matrix.jpg", "popularity": 80.0},
        ]})
    if "search/tv" in url:
        return _FakeResp({"results": []})
    if "omdbapi" in url:
        return _FakeResp({"Search": [
            {"Title": "The Matrix", "Year": "1999", "imdbID": "tt0133093",
             "Poster": "https://img/omdb.jpg", "Type": "movie"},
...
async def test_submit_feedback_rate_limited(db):
    req = _request(db=db, feedback_rate_limiter=RateLimiter(max_requests=1, window_seconds=300))
...
async def test_resolve_suggestion_flags_already_owned(db):
    ...
    cover = _FakeCover([
        {"source": "tmdb", "id": "603", "title": "The Matrix", "year": "1999",
         "media_type": "movie", "poster_url": "http://x"},
        {"source": "tmdb", "id": "1", "title": "Brand New Film", "year": "2026",
...
async def test_submit_suggestion_already_have(db):
    ...
    res = await submit_suggestion(
        SuggestSubmit(query="matrix", choice=SuggestChoice(
            source="tmdb", id="603", title="The Matrix", year="1999")),
        req, user=USER,
    )
...
async def test_submit_suggestion_resolved_new_title(db):
    req = _request(db=db, feedback_rate_limiter=_lenient_limiter())
    res = await submit_suggestion(
        SuggestSubmit(query="inception", choice=SuggestChoice(
...
async def test_submit_suggestion_empty_choice_is_unresolved(db):
    ...
    req = _request(db=db, feedback_rate_limiter=_lenient_limiter())
    res = await submit_suggestion(
        SuggestSubmit(query="mystery", choice=SuggestChoice(title="   ")), req, user=USER)
    assert res["resolution"] == "unresolved"
...
async def test_submit_suggestion_sanitizes_bad_source(db):
    ...
    await submit_suggestion(
        SuggestSubmit(query="x", choice=SuggestChoice(
...
async def test_admin_feedback_list_mark_delete(db):
    ...
    res = await admin_set_feedback_status(fid, StatusUpdate(status="read"), req, user=ADMIN)
    assert res["success"] is True
    assert (await db.list_feedback())[0]["status"] == "read"

    with pytest.raises(HTTPException) as ei:
        await admin_set_feedback_status(fid, StatusUpdate(status="bogus"), req, user=ADMIN)
    assert ei.value.status_code == 400

    with pytest.raises(HTTPException) as ei:
        await admin_set_feedback_status(99999, StatusUpdate(status="read"), req, user=ADMIN)
...
async def test_admin_suggestion_list_mark_delete(db):
    ...
    assert (await admin_set_suggestion_status(sid, StatusUpdate(status="read"), req, user=ADMIN))["success"]
    assert (await db.list_title_suggestions())[0]["status"] == "read"

    with pytest.raises(HTTPException) as ei:
        await admin_set_suggestion_status(99999, StatusUpdate(status="read"), req, user=ADMIN)
    assert ei.value.status_code == 404
