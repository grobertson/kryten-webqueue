...
def _content(
    ...
    *,
    ...
    return {
        ...
        "lead_in_for_uid": lead_in_for_uid,
        "media_type": "cm",
        "media_id": f"m{uid}",
    }
...
def _config(**over):
    base = dict(
        ...
        general=GeneralPromoConfig(
            every_n_items=4, every_m_minutes=20.0, no_repeat=True
        ),
        types={
            ...
            "feature_presentation": PromoTypeConfig(
                enabled=True, order="random", weight=1
            ),
...
async def test_feature_presentation_leadin_before_movie():
    shadow = _FakeShadow(
        [_content(10), _content(20, duration=3600)], now_playing={"uid": 10}
    )
...
async def test_viewers_choice_leadin_before_paid():
    shadow = _FakeShadow(
        [_content(10), _content(20, is_pay=True, duration=300)], now_playing={"uid": 10}
    )
    api = _FakeApiGate(now_playing={"uid": 10})
...
async def test_paid_movie_gets_viewers_choice_not_feature():
    ...
    shadow = _FakeShadow(
        [_content(10), _content(20, is_pay=True, duration=7200)],
        now_playing={"uid": 10},
    )
    api = _FakeApiGate(now_playing={"uid": 10})
    db = _FakeDb(
        pools={"viewers_choice": [_clip("vc1")], "feature_presentation": [_clip("fp1")]}
    )
    d = _director(api, shadow, db)
...
async def test_no_leadin_for_short_free_item():
    shadow = _FakeShadow(
        [_content(10), _content(20, duration=300)], now_playing={"uid": 10}
    )
    api = _FakeApiGate(now_playing={"uid": 10})
    db = _FakeDb(
        pools={"feature_presentation": [_clip("fp1")], "viewers_choice": [_clip("vc1")]}
    )
    d = _director(api, shadow, db)
...
async def test_general_cadence_fires_after_n_items():
    ...
    cfg = _config(
        types={
            "channel_identity": PromoTypeConfig(
                enabled=True, order="sequential", weight=1
            ),
...
async def test_general_cadence_fires_after_m_minutes():
    ...
    cfg = _config(
        general=GeneralPromoConfig(
            every_n_items=99, every_m_minutes=20.0, no_repeat=True
        ),
        types={
            "channel_identity": PromoTypeConfig(
                enabled=True, order="sequential", weight=1
...
async def test_remove_lead_in_for_cleanup():
    shadow = _FakeShadow(
        [
            _content(10),
            _content(
                99, is_promo=True, promo_type="viewers_choice", lead_in_for_uid=20
            ),
