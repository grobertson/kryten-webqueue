...
def test_extract_manifest_token():
    assert (
        fetchurls._extract_manifest_token(
            "https://cms/api/v1/media/cytube/AbC123.json?format=json"
        )
        == "AbC123"
    )
...
async def test_fetch_job_appends_to_playlist(db, monkeypatch):
    ...
    pid = await db.create_saved_playlist(
        name="P", description=None, is_immutable=False, created_by="admin"
    )

    result = await tasks.fetch_job(
        {"url": "http://x", "add_to_playlist": pid}, _ctx(db)
    )
...
async def test_import_section_idempotent(db):
    ...
    info1 = await tasks._import_section_as_playlist(
        ctx, "3.6-3.7-friday", ["cm:t1", "cm:t2"], "admin"
    )
    assert info1["count"] == 2
    # Re-run with one item: same playlist, items replaced (no duplicate playlist).
    info2 = await tasks._import_section_as_playlist(
        ctx, "3.6-3.7-friday", ["cm:t1"], "admin"
    )
    assert info2["id"] == info1["id"]
    playlists = [
        p for p in await db.get_saved_playlists() if p["name"] == "3.6-3.7-friday"
    ]
