...
async def test_append_and_most_recent_playlist(db):
    ...
    first = await db.create_saved_playlist(
        name="First", description=None, is_immutable=False, created_by="admin"
    )
    second = await db.create_saved_playlist(
        name="Second", description=None, is_immutable=False, created_by="admin"
    )
